...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Manually scroll the container to the bottom prevents page jumping
  ...
  return (
    <section className="py-24 bg-navy-900 text-white relative border-t border-navy-800">
      <div className="max-w-6xl mx-auto px-6">
        
        <div className="flex items-center justify-between mb-12">
          <div>
            <h2 className="text-3xl font-display text-white">Infrastructure Command</h2>
            <p className="text-slate-400 font-mono text-sm mt-1">Real-time system telemetry</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Terminal / Log Stream */}
          <div className="lg:col-span-2 bg-navy-950 rounded-lg border border-navy-700 p-1 font-mono text-xs shadow-2xl overflow-hidden flex flex-col h-full min-h-[300px]">
             ...
             >
               {logs.map((log, i) => (
                 <motion.div 
                    ...
                    className={`mb-1.5 ${log.includes("WARN") ? "text-yellow-500" : log.includes("SEC") || log.includes("ERR") ? "text-alert-500" : log.includes("SUCCESS") ? "text-green-400" : "text-slate-300"}`}
                 >
...
const MetricCard: React.FC<{ label: string, value: string, icon: React.ReactNode, sub: string }> = ({ label, value, icon, sub }) => {
  return (
    <motion.div 
      ...
      className="bg-navy-800/50 border border-navy-700 p-5 rounded hover:border-gold-500/50 transition-colors"
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-slate-400 text-xs font-mono uppercase tracking-widest">{label}</span>
        {icon}
      </div>
      <div className="text-2xl font-bold font-mono text-white shadow-gold-500 drop-shadow-[0_0_5px_rgba(255,255,255,0.1)]">
        {value}
      </div>
